...
#[cfg(all(unix, feature = "std", not(target_os = "emscripten"), not(target_vendor = "teaclave")))]
mod fork {
...
#[cfg(not(all(unix, feature = "std", not(target_os = "emscripten"), not(target_vendor = "teaclave"))))]
mod fork {
    pub fn get_fork_counter() -> usize {
        0
    }
